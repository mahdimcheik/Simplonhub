...
namespace SimplonHubApi.Services
{
    public class SeaweedStorageService
    {
        ...
        public SeaweedStorageService(IConfiguration config, ILogger<SeaweedStorageService> logger)
        {
            ...
            _logger.LogInformation($"Initializing SeaweedFS with URL: {EnvironmentVariables.S3_SERVICE_URL}");
        ...
        private async Task EnsureBucketExistsAsync()
        {
            try
            {
                ...
                catch (AmazonS3Exception s3Ex) when (s3Ex.ErrorCode == "NoSuchBucket" || s3Ex.StatusCode == System.Net.HttpStatusCode.NotFound)
                {
            ...
            catch (AmazonS3Exception s3Ex)
            {
                ...
                _logger.LogWarning($"Could not verify bucket existence. Will attempt to create on first upload. SeaweedFS may require filer configuration.");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Unexpected error ensuring bucket exists: {ex.Message}");
                _logger.LogWarning($"Continuing without bucket verification. SeaweedFS will auto-create bucket on first upload if configured properly.");
            }
        }
        ...
        public async Task<string> UploadAsync(IFormFile file)
        {
            try
            {
                ...
                _logger.LogInformation($"Uploading file: {file.FileName} (Size: {file.Length} bytes, ContentType: {file.ContentType})");
            ...
            catch (AmazonS3Exception s3Ex)
            {
                ...
                throw new Exception($"SeaweedFS S3 error: {s3Ex.Message}. Error Code: {s3Ex.ErrorCode}", s3Ex);
            }
            ...
            catch (System.IO.IOException ioEx) when (ioEx.Message.Contains("response ended prematurely") || ioEx.Message.Contains("ResponseEnded"))
            {
                _logger.LogError(ioEx, $"SeaweedFS connection error: {ioEx.Message}");
                _logger.LogError($"This typically means SeaweedFS closed the connection unexpectedly.");
                _logger.LogError($"Possible causes:");
                _logger.LogError($"  1. SeaweedFS S3 gateway is not properly connected to the filer");
                _logger.LogError($"  2. Bucket '{_bucket}' doesn't exist (create with: curl -X POST http://localhost:8888/{_bucket}/?pretty=y -d '{{}}')");
        ...
        public async Task<(Stream Stream, string ContentType)?> ReadAsync(string key)
        {
            try
            {
                ...
                _logger.LogInformation($"File retrieved successfully. ContentType: {response.Headers.ContentType}");
        ...
        public async Task<bool> TestConnectionAsync()
        {
            try
            {
                ...
                _logger.LogInformation($"Connection test successful. Found {buckets.Buckets.Count} buckets.");
                return true;
            }
            catch (Exception ex)
            {
